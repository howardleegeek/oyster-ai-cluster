---
task_id: S03-cli-statemachine
project: pipeline
priority: 1
depends_on: [S01, S02]
modifies: ["dispatch/pipeline/pipeline.py"]
executor: glm
---

## 目标
实现 pipeline.py CLI 入口 + 六层状态机流转逻辑

## 要创建的文件

### dispatch/pipeline/pipeline.py
```python
#!/usr/bin/env python3
"""
...
from db import init_db, create_run, update_run, get_run, get_latest_run, save_layer_result, get_layer_results
...
def run_layer(layer_name, project_config, run_id, prev_results):
    ...
    for attempt in range(1, layer.max_retries + 1):
        ...
        save_layer_result(
            run_id, layer_name, result.status, attempt=attempt,
        ...
        if layer.validate(result):
            ...
            log(f"  {layer_name} FAIL (attempt {attempt}): {result.error or 'validation failed'}")
...
def cmd_run(args):
    ...
    run_id = create_run(project_name, config=pc.__dict__ if hasattr(pc, '__dict__') else {})
    prev_results = {}
    ...
    while i < len(layers):
        ...
        if result is None:
            update_run(run_id, status="FAILED", finished_at=datetime.utcnow().isoformat())
    ...
    update_run(run_id, status="DONE", current_layer="DONE", finished_at=datetime.utcnow().isoformat())
    ...
    summary = {
        ...
        "layers": {k: {"status": v.status, "report": v.report} for k, v in prev_results.items()},
    }
    (report_dir / f"run-{run_id}-summary.json").write_text(json.dumps(summary, indent=2))
...
def cmd_status(args):
    ...
    if args.project:
        ...
        print(f"Project: {args.project} | Run: {run['id']} | Status: {run['status']} | Layer: {run['current_layer']}")
        for r in results:
            print(f"  {r['layer']}: {r['status']} (attempt {r['attempt']})")
    else:
        for name in list_project_names():
            run = get_latest_run(name)
            status = f"run {run['id']} | {run['status']} | {run['current_layer']}" if run else "no runs"
            print(f"  {name}: {status}")
        ...
        if run:
            ...
            for r in results:
                if r["layer"] == args.layer:
                    print(json.dumps(json.loads(r["report"]) if r["report"] else {}, indent=2))
                    return
...
## 验收标准
- [ ] `python3 pipeline.py projects` 列出 5 个项目
- [ ] `python3 pipeline.py status` 显示所有项目状态
- [ ] `python3 pipeline.py run clawvision --layer L1` 能调用 L1（即使 L1 还是 stub）
- [ ] 状态机正确: L4 PASS 跳 L5, L4 FAIL 进 L5 循环

## 不要做
- 不要实现 L1-L6 的真实逻辑（S04-S06 做）
- 不要修改 db.py 或 config.py
- 不要修改 dispatch/ 下已有文件
