...
## 目标
实现 L4 自测层 — API curl 测试 + 页面加载测试 + 端到端流程测试

## 要创建的文件

### dispatch/pipeline/layers/L4_test.py
    ...
    def execute(self, project_config, prev_results: dict) -> LayerResult:
        ...
        try:
            ...
            if be_port:
                ...
                # 健康检查
                r = local.run(f"curl -sf {base}{project_config.backend.health} -w '\\n%{{http_code}}'", timeout=10)
                ...
                if l1 and l1.report:
                    ...
                    for ep in endpoints[:20]:  # 限制 20 个
                        ...
                        path = ep["path"]
                        if method == "GET":
                            r = local.run(f"curl -sf {base}{path} -w '\\n%{{http_code}}'", timeout=10)
                        else:
                            r = local.run(f"curl -sf -X {method} {base}{path} -H 'Content-Type: application/json' -d '{{}}' -w '\\n%{{http_code}}'", timeout=10)

                        passed = r.success or "422" in r.stdout  # 422 = 参数校验正常
                        test_results["api"].append({
                            "endpoint": path,
                            "method": method,
                            "test": "reachable",
            ...
            if fe_port and project_config.test_urls:
                for url in project_config.test_urls:
                    ...
                    r = local.run(f"curl -sf {full_url} -o /dev/null -w '%{{http_code}}'", timeout=10)
            ...
            if be_port and project_config.test_flows:
                for flow in project_config.test_flows:
                    flow_name = flow.get("name", "unnamed") if isinstance(flow, dict) else str(flow)
        ...
        report = {
            "api": {"pass": api_pass, "total": api_total, "details": test_results["api"]},
            "pages": {"pass": page_pass, "total": page_total, "details": test_results["pages"]},
        ...
        if report["s1_bugs"] == 0 and (api_total == 0 or api_pass / api_total >= 0.8) and (page_total == 0 or page_pass / page_total >= 0.8):
            result.finish("PASS", report=report)
        else:
            result.finish("FAIL", report=report, error=f"{len(bugs)} bugs found ({report['s1_bugs']} S1, {report['s2_bugs']} S2)")

        return result
```

## 验收标准
- [ ] 对有后端的项目，测试所有 L1 发现的 endpoint
- [ ] 对有前端的项目，测试所有 test_urls 页面
- [ ] bug 列表包含 severity + description
- [ ] 通过率 < 80% 或有 S1 bug 时判定 FAIL
- [ ] 测试后清理所有启动的进程
