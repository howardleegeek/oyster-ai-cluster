---
task_id: S04-capabilities-scheduling
project: infrastructure
priority: 1
depends_on: []
modifies:
  - dispatch/capabilities.py
executor: glm
---

## 目标
创建能力感知调度模块，根据节点能力分配任务

## 具体改动

### 创建文件: dispatch/capabilities.py
...
## 验收标准
- [ ] 模块可导入 `from dispatch.capabilities import get_matching_nodes`
- [ ] `get_matching_nodes(["browser"])` 返回有 browser 的节点
- [ ] `select_best_node` 能选出合适的节点

## 测试命令
```bash
cd ~/Downloads/dispatch
python3 -c "from capabilities import get_matching_nodes; print(get_matching_nodes(['browser']))"
```
